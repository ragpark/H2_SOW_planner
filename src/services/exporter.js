 ...
 */
export function schemeToMarkdown(detail, spine) {
  ...
  out.push('');
  out.push(
    `**Year ${scheme.yearGroup} ${spine.subjectTitle}** · ${spine.keyStageTitle}` +
  ...
  out.push(`| Curriculum coverage | ${coverage.percent}% of ${spine.title} statements |`);
  ...
  for (const placement of detail.placements) {
    const unit = spine.getUnit(placement.unitId);
    if (!unit) continue;
  ...
  out.push('---');
  out.push('');
  out.push(`Curriculum statements: ${spine.source}.`);
...
export function lessonPlanToMarkdown(lessonId, { spine, scheme = null, note = null } = {}) {
  const resolved = spine || registry.list().find((s) => s.getLesson(lessonId));
  if (!resolved) return null;
  const lesson = resolved.getLesson(lessonId);
  if (!lesson) return null;
  const unit = resolved.getUnit(lesson.unitId);
  const practical = lesson.practical ? resolved.getPractical(lesson.practical) : null;
  const out = [];
  ...
  for (const id of [...(lesson.ncRefs || []), ...(lesson.wsRefs || [])]) {
    const s = resolved.getStatement(id);
